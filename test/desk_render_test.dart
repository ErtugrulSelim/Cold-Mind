...
void main() {
  ...
  testWidgets('the paywall draws its plans and the price of each', (
    ...
    final plans = await const UnconfiguredStore().plans();
    ...
    // Which tier buys hints is the only thing separating two plans of the
    // same length, so a card that does not say so is a card nobody can
    // choose between.
    expect(
      find.text(common.c('paywall.plan_hints')),
      findsNWidgets(plans.where((p) => p.includesHints).length),
      reason: 'the plans that include hints never said so',
    );
