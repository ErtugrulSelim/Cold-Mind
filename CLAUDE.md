...
`assets/icons/app_logo.png` is the one source for everything the player sees
before the game does — the launcher icon and the native splash both come off
it, and replacing it means rerunning the three commands below in order.
...
against the logo's own corner tone rather than the device register's graphite,
because the splash centres the image on a flat colour and any other colour
leaves a visible square around it. Configured under `flutter_native_splash:`
in `pubspec.yaml`. It writes directly into the native Android and iOS projects
(launch backgrounds, styles, `LaunchScreen.storyboard`, `Info.plist`).

The launcher icon needs the logo reframed first, which is what
`tools/gen_icon.dart` does: a centre crop for the legacy square, and the whole
logo scaled into the **safe circle** for the adaptive foreground. An adaptive
icon is 108dp with only the middle 72dp guaranteed, and `flutter_launcher_icons`
insets nothing — it scales what it is handed to fill the canvas — so a
full-bleed image loses the top of the phone and the base of the wordmark on
every launcher that masks to a circle.

```bash
dart run tools/gen_icon.dart              # logo → the two icon sources
dart run flutter_launcher_icons
dart run flutter_native_splash:create
```

None of this is reachable by `flutter test`, so check it on a real device or
simulator rather than trusting the suite for it.
